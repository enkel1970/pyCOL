...
class CameraThread(QThread):
    ...
    def run(self):
        ...
        if platform_name == "windows":
            ...
            print("Using default backend for macOS")
...
class MainWindow(QMainWindow):
    ...
    def connect_overlay_controls(self):
        ...
        self.ui.sliderRadius_1.valueChanged.connect(
            lambda val: (
                self.update_circle(0, radius=val),
                self.ui.lbl_r1.setText(str(val)),
            )
        )
        self.ui.sliderThickness_1.valueChanged.connect(
            lambda val: (
                self.update_circle(0, thickness=val),
                self.ui.lbl_t1.setText(str(val)),
            )
        )
        self.ui.checkBox_1.stateChanged.connect(
            lambda state: self.update_circle(0, visible=(state == 2))
        )

        # Circle 2 and checkbox
        self.ui.sliderRadius_2.valueChanged.connect(
            lambda val: (
                self.update_circle(1, radius=val),
                self.ui.lbl_r2.setText(str(val)),
            )
        )
        self.ui.sliderThickness_2.valueChanged.connect(
            lambda val: (
                self.update_circle(1, thickness=val),
                self.ui.lbl_t2.setText(str(val)),
            )
        )
        self.ui.checkBox_2.stateChanged.connect(
            lambda state: self.update_circle(1, visible=(state == 2))
        )

        # Circle 3 and checkbox
        self.ui.sliderRadius_3.valueChanged.connect(
            lambda val: (
                self.update_circle(2, radius=val),
                self.ui.lbl_r3.setText(str(val)),
            )
        )
        self.ui.sliderThickness_3.valueChanged.connect(
            lambda val: (
                self.update_circle(2, thickness=val),
                self.ui.lbl_t3.setText(str(val)),
            )
        )
        self.ui.checkBox_3.stateChanged.connect(
            lambda state: self.update_circle(2, visible=(state == 2))
        )
    ...
    def start_camera(self):
        ...
        if camera_index is None:
            ...
            msg.setText("No compatible camera found.")
            ...
            self.video_window.set_circle_property(
                0, "radius", self.ui.sliderRadius_1.value()
            )
            self.video_window.set_circle_property(
                0, "thickness", self.ui.sliderThickness_1.value()
            )
            self.video_window.set_circle_property(
                0, "visible", self.ui.checkBox_1.isChecked()
            )
            self.video_window.set_circle_property(
                1, "radius", self.ui.sliderRadius_2.value()
            )
            self.video_window.set_circle_property(
                1, "thickness", self.ui.sliderThickness_2.value()
            )
            self.video_window.set_circle_property(
                1, "visible", self.ui.checkBox_2.isChecked()
            )
            self.video_window.set_circle_property(
                2, "radius", self.ui.sliderRadius_3.value()
            )
            self.video_window.set_circle_property(
                2, "thickness", self.ui.sliderThickness_3.value()
            )
            self.video_window.set_circle_property(
                2, "visible", self.ui.checkBox_3.isChecked()
            )
            self.video_window.set_cross_property(
                "length", self.ui.sliderCrossLength.value()
            )
            self.video_window.set_cross_property(
                "thickness", self.ui.sliderThicknessCross.value()
            )
            self.video_window.set_cross_property(
                "angle", self.ui.sliderCrossAngle.value()
            )
            self.video_window.set_cross_property(
                "visible", self.ui.checkBox_4.isChecked()
            )
            self.video_window.set_cross_property(
                "color",
                getattr(self.ui, "label_color_4")
                .palette()
                .color(getattr(self.ui, "label_color_4").backgroundRole()),
            )
            self.video_window.show()
            self.ui.btnOpenCamera.setEnabled(
                False
    ...
    def update_circle(
        ...
        if self.video_window:
            ...
            print(
                f"Updated circle {index} with radius={radius}, color={color}, thickness={thickness}, visible={visible}"
            )
